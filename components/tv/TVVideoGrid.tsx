...
const TVVideoGrid: React.FC<TVVideoGridProps> = ({ videos, client, onSelect, initialFocusedIndex = 0 }) => {
      ...
      <div className="grid grid-cols-8 gap-3 px-4 pb-[50vh]">
        {videos.map((item, index) => {
          ...
          return (
            ...
            >
              {posterSrc ? (
                <img src={posterSrc} alt={item.Name} className="w-full h-full object-cover transition-transform duration-700 group-focus:scale-110" />
              ) : (
              ...
              <div className="absolute bottom-0 left-0 right-0 p-2 translate-y-4 opacity-0 group-focus:translate-y-0 group-focus:opacity-100 transition-all duration-500 ease-out">
                  <p className="text-white font-black text-[10px] line-clamp-2 leading-tight">{item.Name}</p>
                  <p className="text-indigo-400 text-[7px] mt-1 font-black uppercase tracking-widest">{item.ProductionYear || 'Content'}</p>
              </div>
            </div>
          );
        })}
