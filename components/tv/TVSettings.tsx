  ...
  return (
    <div className="absolute inset-0 flex animate-in fade-in duration-300 z-40 bg-black settings-panel h-full">
      ...
      <div className="w-40 sm:w-48 lg:w-56 bg-white/5 p-4 sm:p-6 space-y-2 border-r border-white/5 settings-tabs h-full shrink-0">
          ...
          <TabButton label={t.account} active={activeTab === 'account'} onFocus={() => handleTabFocus('account')} icon={<User size={14} />} />
          <TabButton label={t.libs} active={activeTab === 'libraries'} onFocus={() => handleTabFocus('libraries')} icon={<Library size={14} />} />
          <TabButton label={t.display} active={activeTab === 'display'} onFocus={() => handleTabFocus('display')} icon={<Monitor size={14} />} />
          <TabButton label={t.about} active={activeTab === 'about'} onFocus={() => handleTabFocus('about')} icon={<Info size={14} />} />
      </div>
      ...
      <div className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto no-scrollbar settings-content h-full bg-gradient-to-br from-zinc-900/10 to-transparent">
          {activeTab === 'account' && (
              <div className="space-y-4 sm:space-y-6 max-w-md sm:max-w-lg animate-in slide-in-from-right-4 duration-500">
                  <h3 className="text-lg sm:text-xl font-black opacity-40 uppercase tracking-widest">{t.account}</h3>
          ...
          {activeTab === 'libraries' && (
              <div className="space-y-4 sm:space-y-6 animate-in slide-in-from-right-4 duration-500">
                  <h3 className="text-lg sm:text-xl font-black opacity-40 uppercase tracking-widest">{t.libs}</h3>
          ...
          {activeTab === 'display' && (
              <div className="space-y-4 sm:space-y-6 animate-in slide-in-from-right-4 duration-500 max-w-md sm:max-w-lg">
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:gap-3">
                      <FilterCard label="竖屏" active={orientationMode === 'vertical'} onClick={() => onOrientationChange('vertical')} icon={<Smartphone size={14} />} />
                      <FilterCard label="横屏" active={orientationMode === 'horizontal'} onClick={() => onOrientationChange('horizontal')} icon={<Monitor size={14} />} />
                      <FilterCard label="全部" active={orientationMode === 'both'} onClick={() => onOrientationChange('both')} icon={<Square size={14} />} />
                  </div>
...
function TabButton({ label, active, onFocus, icon }: { label: string, active: boolean, onFocus: () => void, icon: any }) {
    return (
        <button 
...
function FilterCard({ label, active, onClick, icon }: { label: string, active: boolean, onClick: () => void, icon: any }) {
    return (
