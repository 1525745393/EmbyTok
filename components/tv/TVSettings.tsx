          ...
          <div className="space-y-4 sm:space-y-6 max-w-md sm:max-w-lg animate-in slide-in-from-right-4 duration-500">
            ...
            <div className="bg-zinc-900/80 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-white/5 space-y-3 sm:space-y-4">
              ...
              <button
                tabIndex={0}
                onClick={onLogout}
                className="w-full py-2 sm:py-3 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-lg sm:rounded-xl font-bold text-xs transition-all outline-none focus:ring-2 focus:ring-red-500"
              >
                {t.tvSettings.logout}
              </button>
            </div>
          ...
          <div className="space-y-4 sm:space-y-6 animate-in slide-in-from-right-4 duration-500 max-w-md sm:max-w-lg">
            ...
            >
              <div className="text-sm font-bold text-white">
                {t.tvSettings.currentLang(language === 'zh' ? '简体中文' : 'English')}
              </div>
            ...
            <h3 className="text-lg sm:text-xl font-black opacity-40 uppercase tracking-widest pt-3 sm:pt-4">
              {t.tvSettings.display}
            </h3>
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
              <FilterCard
                label={t.tvSettings.vertical}
                active={orientationMode === 'vertical'}
                onClick={() => onOrientationChange('vertical')}
                icon={Smartphone}
              />
              <FilterCard
                label={t.tvSettings.horizontal}
                active={orientationMode === 'horizontal'}
                onClick={() => onOrientationChange('horizontal')}
                icon={Monitor}
              />
              <FilterCard
                label={t.tvSettings.both}
        ...
        {activeTab === 'about' && (
          <div className="space-y-4 sm:space-y-6 animate-in slide-in-from-right-4 duration-500 max-w-md sm:max-w-lg">
            <div className="bg-gradient-to-br from-indigo-600/10 to-transparent p-4 sm:p-6 lg:p-8 rounded-xl sm:rounded-2xl border border-white/5 text-white">
              ...
              <p className="text-xs text-white/70 leading-relaxed mb-4 sm:mb-6">
                {t.tvSettings.tvDesc}
              </p>
              <div className="flex gap-2 sm:gap-3">
                <div className="bg-white/5 px-3 sm:px-4 py-1 sm:py-1.5 rounded-full text-[10px] font-black border border-white/5 uppercase tracking-widest">
                  {t.librarySelect.version(version)}
                </div>
              </div>
            </div>
          </div>
...
function FilterCard({
  ...
  return (
    ...
    >
      <div className={`mb-1 sm:mb-2 ${active ? 'text-black' : 'text-indigo-400'}`}>
        <Icon size={14} />
      </div>
