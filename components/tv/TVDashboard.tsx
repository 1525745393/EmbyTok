  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const latest = await client.getVideos(undefined, null, 'latest', 0, 15, 'horizontal', includeIds);
          ...
          {libraries.map((lib) => {
            ...
            return (
                ...
                >
                    {coverSrc ? (
                        <>
                            <img src={coverSrc} className="w-full h-full object-cover transition-transform duration-1000 group-focus:scale-110" alt="" />
      ...
      <section className="animate-in slide-in-from-bottom-4 duration-700 delay-100">
        ...
        <div className="flex gap-3 overflow-x-auto no-scrollbar px-8 py-2">
          {latestItems.map((item) => (
            <DashboardCard key={item.Id} item={item} client={client} onClick={() => onSelectVideo(item)} />
          ))}
        </div>
      </section>
    </div>
...
function DashboardCard({ item, client, onClick }: { item: EmbyItem, client: any, onClick: () => void }) {
    ...
    return (
        ...
        >
            {posterSrc ? <img src={posterSrc} className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full flex items-center justify-center text-[6px] text-white/20 uppercase font-black p-2 text-center">{item.Name}</div>}
            ...
            <div className="absolute bottom-2 left-2 right-2 translate-y-2 opacity-0 group-focus:translate-y-0 group-focus:opacity-100 transition-all text-left">
                <p className="text-[8px] font-black line-clamp-2 leading-tight text-white drop-shadow-lg">{item.Name}</p>
            </div>
        </button>
    );
}
