...
const TVVideoPlayer: React.FC<TVVideoPlayerProps> = ({ videos, initialIndex, onBack, client, libraryName, language }) => {
  ...
  const changeVideo = useCallback((offset: number) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (!showInfoOverlay) {
          switch(e.key) {
              case 'ArrowUp': e.preventDefault(); changeVideo(-1); break;
              case 'ArrowDown': e.preventDefault(); changeVideo(1); break;
              case 'ArrowLeft': if (videoRef.current) videoRef.current.currentTime -= 15; break;
              case 'ArrowRight': if (videoRef.current) videoRef.current.currentTime += 15; break;
              case 'Enter': 
              case ' ': e.preventDefault(); videoRef.current?.pause(); setShowInfoOverlay(true); break;
              case 'Escape':
              case 'Backspace': 
                e.preventDefault(); 
          ...
          if (e.key === 'Enter' || e.key === ' ') {
              if (document.activeElement?.id === 'tv-player-play-btn' || document.activeElement?.tagName !== 'BUTTON') {
                  e.preventDefault();
  ...
  return (
    <div className="h-full w-full bg-black relative flex items-center justify-center overflow-hidden font-sans">
      ...
      <div className={`absolute inset-0 z-20 flex flex-col transition-all duration-1000 ease-out ${showInfoOverlay ? 'opacity-100 scale-100 backdrop-blur-xl bg-black/40' : 'opacity-0 pointer-events-none scale-110 backdrop-blur-none bg-transparent'}`}>
          ...
          <div className="absolute left-10 top-1/2 -translate-y-1/2 z-10">
              {posterSrc && (
                  <div className="relative animate-in zoom-in-95 duration-1000">
                      ...
                      <img src={posterSrc} className={`w-[240px] aspect-[2/3] object-cover rounded-[20px] shadow-[0_40px_100px_-20px_rgba(0,0,0,1)] border border-white/5 relative z-10 transition-transform duration-[3500ms] ease-out ${showInfoOverlay ? 'scale-105' : 'scale-100'}`} alt="Poster" />
                  </div>
              )}
          </div>
          <div className="absolute right-10 left-[340px] top-0 bottom-0 flex flex-col justify-center items-end text-right z-10">
              <div className="absolute top-8 right-0">
                  <button onClick={onBack} tabIndex={0} className="player-action-btn p-3 bg-white/5 border border-white/10 rounded-full hover:bg-white hover:text-black outline-none focus:ring-2 focus:ring-white transition-all shadow-xl"><ChevronLeft size={20} /></button>
              </div>
              <div className="animate-in slide-in-from-right-8 duration-700 w-full">
                  <h1 className="text-4xl font-black mb-2 tracking-tight text-white drop-shadow-2xl uppercase leading-tight">{currentItem.Name}</h1>
                  <div className="flex gap-3 text-indigo-400 font-black uppercase tracking-[0.3em] text-[8px] justify-end">
                      <span className="bg-white/10 px-1.5 py-0.5 rounded text-white">{currentItem.ProductionYear}</span>
          ...
          <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-white/5 overflow-hidden">
              <div className={`h-full bg-indigo-600 transition-all duration-[2500ms] linear ${showInfoOverlay ? 'w-full' : 'w-0'}`} />
          </div>
      </div>
    </div>
  );
