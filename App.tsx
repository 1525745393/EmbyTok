...
function App() {
  ...
  const fetchLibraries = async () => {
    if (!client) return;
    try { setLibraries(await client.getLibraries()); } catch (e) {}
  };
  ...
  return (
    <div className="relative h-[100dvh] w-full bg-black overflow-hidden font-sans text-white">
      ...
      {/* 统一响应式顶部导航栏 */}
      <div className={`absolute top-0 left-0 right-0 z-40 h-16 bg-gradient-to-b from-black/95 to-transparent backdrop-blur-sm flex items-center justify-between px-3 pt-2 transition-opacity duration-300 ${isAutoPlay ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
      ...
      <div className="w-full h-full bg-black">
        {viewMode === 'grid' ? (
            <VideoGrid 
                videos={videos} client={client} isLoading={loading} feedType={feedType} hasMore={hasMore}
                onSelect={(idx) => { setCurrentIndex(idx); setViewMode('feed'); }}
            ...
            <VideoFeed 
                videos={videos} client={client} onRefresh={() => loadVideos(true)} isLoading={loading}
                favoriteIds={favoriteIds} onToggleFavorite={handleToggleFavorite}
                initialIndex={currentIndex} onIndexChange={setCurrentIndex} isMuted={isMuted} onToggleMute={() => setIsMuted(!isMuted)}
                feedType={feedType} hasMore={hasMore} onLoadMore={() => loadVideos(false)} isAutoPlay={isAutoPlay} onToggleAutoPlay={() => setIsAutoPlay(!isAutoPlay)}
            />
        )}
      </div>
      ...
      <LibrarySelect 
        ...
        onSelect={setSelectedLib} hiddenLibIds={hiddenLibIds} onToggleHidden={(id) => {
        ...
        }} onLogout={handleLogout} serverUrl={config.url} username={config.username}
        orientationMode={orientationMode} onOrientationChange={setOrientationMode}
      />
    </div>
  );
}
