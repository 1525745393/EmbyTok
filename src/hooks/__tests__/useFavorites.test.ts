...
describe('useFavorites Hook', () => {
  ...
  it('renames a collection', () => {
    ...
    expect(result.current.collections.find(c => c.id === newCollection.id)?.name).toBe('New Name');
  });
  ...
  it('adds an item to favorites in specific collection', () => {
    ...
    expect(result.current.collections.find(c => c.id === newCollection.id)?.itemIds).toContain('item456');
  });
  ...
  it('removes an item from all collections when no collection specified', () => {
    ...
    expect(result.current.collections.find(c => c.id === newCollection.id)?.itemIds).not.toContain('item101');
  });
  ...
  it('removes an item from specific collection', () => {
    ...
    expect(result.current.collections.find(c => c.id === newCollection.id)?.itemIds).not.toContain('item202');
  });
